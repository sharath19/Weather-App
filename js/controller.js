...
myApp.controller('Rendersdata', function renderdata($scope,$http,$log){
	...
	$scope.submit = function() {
	    ...
	    d3.selectAll('svg')
	    	.remove();
	    var weatherUrl = 'http://api.openweathermap.org/data/2.5/forecast/daily?q='+$scope.text+'&cnt=16&appid=73b59da4a0aef80899ab8e4e8c1dae3a';
		$http.get(weatherUrl).success(function(data) {
			var bardata = [];
			
			for(var i =0; i< (data.list).length; i++) {
				$log.debug(Math.floor(data.list[i].temp.max));
				bardata.push(((data.list[i].temp.min)*9/5)-459.67);
			}	
